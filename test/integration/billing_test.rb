...
class BillingTest < ActionDispatch::IntegrationTest
  ...
  test "checkout uses the selected configured Stripe price" do
    ...
    assert_equal "https://kilnwood-roast.api.holode.xyz/billing?checkout=success", captured[:success_url]
    assert_equal "https://kilnwood-roast.api.holode.xyz/pricing?interval=year", captured[:cancel_url]
  ...
  test "mixed manual and subscription state still exposes the portal" do
    ...
    assert_equal "https://kilnwood-roast.api.holode.xyz/billing", portal_arguments[:return_url]
  end
end
